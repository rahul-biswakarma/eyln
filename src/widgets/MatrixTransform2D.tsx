...
export function MatrixTransform2D() {
  ...
  useEffect(() => {
    ...
    const toScreen = (x: number, y: number): [number, number] => [cx + x * unit, cy - y * unit];
    ...
    for (let i = -6; i <= 6; i++) {
      ...
      p = toScreen(...apply(i, 6)); ctx.lineTo(p[0], p[1]); ctx.stroke();
      p = toScreen(...apply(-6, i)); ctx.beginPath(); ctx.moveTo(p[0], p[1]);
    ...
    const drawBasis = (vx: number, vy: number, color: string, label: string) => {
      ...
      ctx.beginPath(); ctx.moveTo(cx, cy); ctx.lineTo(s[0], s[1]); ctx.stroke();
  ...
  const slider = (key: keyof typeof m, label: string) => (
    <div className="ctl">
      <label>{label}</label>
      ...
      <div className="wbody">
        ...
        <div className="row" style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 10 }}>
          {Object.entries(presets).map(([k, v]) => (
            <button key={k} className="btn" onClick={() => setM(v)}>{k}</button>
          ))}
        </div>
