...
export function TerrainField() {
  ...
  return (
    <>
      <WebGPUCanvas
        title="Heightfield terrain — a grid of ~15k triangles"
        height={360}
        setup={(gpu) => {
          ...
          function buildMesh() {
            ...
            const H = (i: number, j: number) =>
              fbm(i / N, j / N, { kind: "perlin", octaves, frequency: freq, lacunarity: 2, gain: 0.5, seed }) * 1.2;
            for (let j = 0; j <= N; j++) for (let i = 0; i <= N; i++) heights[j * (N + 1) + i] = H(i, j);

            const pos = (i: number, j: number): [number, number, number] => [
          ...
          const loop = startLoop((t) => {
            ...
            const r = 5.5;
            const eye: [number, number, number] = [Math.cos(t * 0.25) * r, 3.2, Math.sin(t * 0.25) * r];
        ...
        <div className="wbody">
          <div className="controls">
            <div className="ctl">
              ...
              <input type="range" min={1} max={8} step={0.5} defaultValue={3}
                onChange={(e) => { params.current.freq = +e.target.value; rebuild.current?.(); force((x) => x + 1); }} />
              <output>{params.current.freq.toFixed(1)}</output>
            </div>
            <div className="ctl">
              <label>octaves</label>
              <input type="range" min={1} max={8} step={1} defaultValue={5}
                onChange={(e) => { params.current.octaves = +e.target.value; rebuild.current?.(); force((x) => x + 1); }} />
              <output>{params.current.octaves}</output>
            </div>
            <div className="ctl">
              <label>seed</label>
              <input type="range" min={1} max={12} step={1} defaultValue={3}
                onChange={(e) => { params.current.seed = +e.target.value; rebuild.current?.(); force((x) => x + 1); }} />
