...
export function noteKind(note: Note): NoteKind {
    ...
    if (note.tags.includes("audio"))
        return "voice";
    if (note.tags.includes("quote"))
        return "quote";
    if (note.tags.includes("vocab"))
        return "vocab";
    if (note.tags.includes("idea"))
        return "idea";
    if (note.tags.includes("pinned"))
        return "ai";
    if (note.tags.includes("formula") || /\$\$?[^$]+\$\$?/.test(note.body))
        return "formula";
    if (note.tags.includes("code") || /```/.test(note.body))
        return "code";
    return "note";
}
export const NOTE_KIND_META: Record<NoteKind, {
    label: string;
    accent: string;
}> = {
...
export function noteCodeBlock(note: Note): {
    lang: string;
    code: string;
} | null {
