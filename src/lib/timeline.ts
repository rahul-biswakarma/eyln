/** Buckets timestamped items into Today / Yesterday / Last Week / Earlier groups. */
export function groupByTimeline<T>(items: T[], getTime: (item: T) => number, now: number): [string, T[]][] {
  ...
  for (const item of items) {
    ...
    else if (t >= weekAgo) buckets["Last Week"].push(item);
