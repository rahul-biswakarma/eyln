/** The set of content kinds the capture workspace can classify into. */
export type CaptureKind = "note" | "formula" | "code" | "quote" | "vocab" | "mistake";

/**
 * Lightweight heuristic classifier — approximates "AI auto-detection" without
 * a network call. Order matters: more specific signals are checked first.
 */
export function detectCaptureKind(text: string): CaptureKind {
  ...
  if (/\$\$[^$]+\$\$|\\[a-zA-Z]+\{|\\(frac|sqrt|sum|int|vec|hat)\b/.test(t)) return "formula";
  if (/\bmistake\b/i.test(t) && /\bcorrect(ion)?\b/i.test(t)) return "mistake";
  if (/^["“].+["”]$/.test(t) || /^["“]/.test(t)) return "quote";
  if (/^[a-zA-Z][a-zA-Z\s'-]{1,28}\s[—-]\s.{6,}/.test(t) && t.split("\n").length === 1) return "vocab";
...
/** Builds the final note body + tags for a capture, given the kind and raw field values. */
export function buildCapture(
  kind: CaptureKind,
  fields: { text: string; source: string; formula: string }
): { body: string; tags: string[] } | null {
  const text = fields.text.trim();

  if (kind === "formula") {
    if (!fields.formula.trim()) return null;
    return { body: `$$\n${fields.formula.trim()}\n$$`, tags: ["formula"] };
  }
  if (kind === "code") {
    if (!text) return null;
