...
function BFS() {
    return (<div className="prose">
      ...
      <Code lang="ts" filename="bfs.ts" code={`function shortestPath(graph: number[][], src: number, dst: number): number {
...
function DFSTopo() {
    return (<div className="prose">
      ...
      <Code lang="ts" filename="dfs.ts" code={`function dfs(graph: number[][], u: number, visited: boolean[]): void {
  visited[u] = true;
  for (const v of graph[u]) {
    if (!visited[v]) dfs(graph, v, visited);   // recurse deeper
      ...
      <Code lang="ts" filename="toposort.ts" code={`function topoSort(graph: number[][]): number[] {
...
function Dijkstra() {
    return (<div className="prose">
      ...
      <Code lang="ts" filename="dijkstra.ts" code={`// graph[u] = array of [neighbor, weight]. Uses a min-heap 'pq'.
...
function UnionFind() {
    return (<div className="prose">
      ...
      <Code lang="ts" filename="dsu.ts" code={`class DSU {
            ...
            exercises: [
                {
                    id: "matrix-space",
                    kind: "numeric",
                    prompt: "A graph has 2,000 vertices and 6,000 directed edges. How many cells does its adjacency matrix have? Enter the number.",
                    starter: "",
                    validate: (s) => Math.abs(parseFloat(s) - 4000000) < 0.5
            ...
            exercises: [
                {
                    id: "bfs-dist",
                    kind: "open",
                    prompt: "You run BFS from node A. Node X is enqueued while processing a node at distance 3. What is X's shortest-path distance from A, and why can BFS guarantee it?",
                    starter: "",
                    rubric: "Full credit: distance 4 (one more than the node at distance 3), and BFS guarantees it because it explores strictly in order of increasing distance, so X's first discovery is along a shortest path. Partial: correct number but weak justification.",
            ...
            exercises: [
                {
                    id: "relax",
                    kind: "open",
                    prompt: "The best known distance to vertex v is 10. We pop u with dist[u] = 7, and there is an edge u -> v of weight 2. Does Dijkstra update dist[v]? To what, and what is this step called?",
                    starter: "",
                    rubric: "Full credit: yes, because 7 + 2 = 9 < 10, so dist[v] becomes 9; the step is called edge relaxation. Partial: correct value but does not name relaxation, or vice versa.",
