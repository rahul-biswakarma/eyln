...
function HashFunctions() {
    return (<div className="prose">
      ...
      <Code lang="ts" filename="hash.ts" code={`// A classic polynomial (Java-style) string hash: h = s[0]*31^{k-1} + ... + s[k-1].
...
function Collisions() {
    return (<div className="prose">
      ...
      <Code lang="ts" filename="probe.ts" code={`// Open addressing with linear probing (insert). EMPTY marks a free slot.
...
function LoadFactor() {
    return (<div className="prose">
      ...
      <Code lang="ts" filename="resize.ts" code={`class HashSet {
...
function MapsInPractice() {
    return (<div className="prose">
      ...
      <Code lang="ts" filename="when-hash.ts" code={`// Membership / dedup where order doesn't matter → hashing wins.
...
function HashPatterns() {
    return (<div className="prose">
      ...
      <Code lang="ts" filename="patterns.ts" code={`// 1) Frequency counting — anagram test in O(n).
...
export const dsaHashing: Module = {
    ...
    icon: "🗄️",
    track: "dsa",
    blurb: "Turning keys into array indices: hash functions, collision resolution, load factor and resizing, and the frequency/dedup/two-sum patterns.",
