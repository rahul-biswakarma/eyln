  ...
  return (
    <div className={rw.library}>
      <header className={rw.libHeader}>
        ...
        <button className={rw.captureBtn} onClick={() => setAdding((v) => !v)}><PlusIcon size={13} weight="bold" /> New Project</button>
      </header>
      ...
      {projects.length === 0 ? (
        <div className={rw.empty}>
          ...
          <h4 className={rw.emptyH4}>No projects yet</h4>
          <p className={rw.emptyP}>Create a project to collect its architecture notes, code, ideas and references.</p>
        </div>
      ) : (
    ...
    <button className={rw.card} onClick={onOpen}>
      ...
      <div className={rw.cardBody}>
        ...
        <div className={rw.cardStats}>
          <span className={rw.cardStatItem}><StackIcon size={12} /> {stats.architectureCount}</span>
          <span className={rw.cardStatItem}><CodeIcon size={12} /> {stats.codeCount}</span>
          <span className={rw.cardStatItem}><LightbulbIcon size={12} /> {stats.ideaCount}</span>
          <span className={rw.cardStatItem}><LinkSimpleIcon size={12} /> {stats.referenceCount}</span>
  ...
  return (
    <div className={rw.detail}>
      ...
      <header className={rw.hero}>
        <span className={rw.spaceIcLg}><StackIcon size={30} weight="duotone" /></span>
        <div className={rw.heroMain}>
          <h1 className={rw.heroMainH1}>{project.title}</h1>
          <span className={rw.heroSub}>Project</span>
          <div className={rw.heroControls}>
            ...
            <Popover>
              ...
              <PopoverContent align="start" style={{ width: 150 }}>
                <div className={rw.menu}><button className={rw.menuBtn(true)} onClick={() => { deleteProject(project.id); onBack(); }}><TrashIcon size={12} /> Delete project</button></div>
              </PopoverContent>
            </Popover>
          </div>
        </div>
      ...
      <Tabs defaultValue="overview" className="rw-tabs">
        ...
        <TabsContent className={rw.tabsContent} value="overview"><OverviewTab project={project} caps={caps} stats={stats} now={now} /></TabsContent>
        <TabsContent className={rw.tabsContent} value="architecture"><NotesTab notes={caps.architecture} now={now} empty="No architecture notes yet" /></TabsContent>
        <TabsContent className={rw.tabsContent} value="code"><CodeTab notes={caps.code} now={now} /></TabsContent>
        <TabsContent className={rw.tabsContent} value="ideas"><NotesTab notes={caps.ideas} now={now} empty="No ideas captured yet" /></TabsContent>
        <TabsContent className={rw.tabsContent} value="references"><ReferencesTab notes={caps.references} now={now} /></TabsContent>
        <TabsContent className={rw.tabsContent} value="timeline"><TimelineTab caps={caps} now={now} /></TabsContent>
      </Tabs>
    </div>
  );
}
...
function OverviewTab({ project, caps, stats, now }: { project: Project; caps: ProjectCaptures; stats: ProjectStats; now: number }) {
  ...
  return (
    <div className={rw.overview}>
      {project.description && <section className={rw.panel}><h4 className={rw.panelH4}>About</h4><p className={rw.panelLine}>{project.description}</p></section>}
      <section className={rw.panel}>
        <h4 className={rw.panelH4}>Contents</h4>
        <div className={rw.statgrid}>
          <div><span className={rw.statgridV}>{stats.architectureCount}</span><span className={rw.statgridL}>Architecture</span></div>
          <div><span className={rw.statgridV}>{stats.codeCount}</span><span className={rw.statgridL}>Code</span></div>
          <div><span className={rw.statgridV}>{stats.ideaCount}</span><span className={rw.statgridL}>Ideas</span></div>
          <div><span className={rw.statgridV}>{stats.referenceCount}</span><span className={rw.statgridL}>References</span></div>
        </div>
      </section>
      <section className={rw.panel}>
        <h4 className={rw.panelH4}>Recent activity</h4>
        {recent.length ? recent.map((n) => (
          <div key={n.id} className={rw.miniNote}><strong>{n.title || n.body.replace(/```/g, "").slice(0, 60)}</strong><span>{relativeDay(n.createdAt, now)}</span></div>
        )) : <p className={rw.panelLineMuted}>Nothing captured yet.</p>}
      </section>
      <section className={rw.panel}><h4 className={rw.panelH4}>Last activity</h4><p className={rw.panelLine}>{stats.lastActivity ? formatDate(stats.lastActivity) : "—"}</p></section>
    </div>
  );
}
...
function NotesTab({ notes, now, empty }: { notes: Note[]; now: number; empty: string }) {
  ...
  return (
    <div className={rw.cardsCol}>
      {notes.map((n) => (
        <div key={n.id} className={rw.noteCard}>
          {n.title && <span className={rw.noteTitle}>{n.title}</span>}
          <p className={rw.noteBody}>{n.body}</p>
  ...
  return (
    <div className={rw.cardsCol}>
      {notes.map((n) => {
        ...
        return (
          <article key={n.id} className={rw.noteCard}>
            {n.title && <span className={rw.noteTitle}>{n.title}</span>}
...
function ReferencesTab({ notes }: { notes: Note[]; now: number }) {
  ...
  return (
    <div className={rw.notes}>
      {notes.map((n) => (
        <div key={n.id} className={rw.searchHit}>
          <span className={rw.hitIc}><LinkSimpleIcon size={14} /></span>
          <span className={rw.hitBody}>
            {n.title && <span className={rw.hitText}>{n.title}</span>}
            {isUrl(n.body)
              ? <a className={rw.hitBook} href={n.body.trim()} target="_blank" rel="noreferrer" onClick={(e) => e.stopPropagation()}>{n.body.trim()}</a>
              : <span className={rw.hitBook}>{n.body}</span>}
          </span>
          <button className={rw.iconBtn()} onClick={() => deleteNote(n.id)} aria-label="Delete"><TrashIcon size={12} /></button>
        </div>
      ))}
    ...
    <div className={rw.timeline}>
      {events.map((e, i) => (
        <div key={i} className={`${rw.tlEvent} k-${e.kind}`}>
          <span className={rw.tlIc}>{P_EVENT_ICON[e.kind]}</span>
