...
export const modules: Module[] = tracks.flatMap((t) => topoSort(raw.filter((m) => (m.track ?? DEFAULT_TRACK) === t.id)));
...
export function moduleHasQuestionary(module: Module): boolean {
    return module.lessons.some((l) => (l.quiz?.questions.length ?? 0) > 0 || (l.exercises?.length ?? 0) > 0);
}
...
export function moduleDifficulty(module: Module): Difficulty {
    ...
    if (depth === 0)
        return { level: 1, label: "Foundational" };
    if (depth === 1)
        return { level: 2, label: "Intermediate" };
...
export function nextLessonInTrack(trackId: TrackId, done: Record<string, boolean>): LessonRef | undefined {
    const lessons = lessonsForTrack(trackId);
    return (lessons.find((r) => !done[lessonKey(r.module.id, r.lesson.id)]) ?? lessons[0]);
}
